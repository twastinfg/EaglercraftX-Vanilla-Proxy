...
export class PluginManager extends EventEmitter {
    constructor(loadDir) {
        ...
        this.Motd = Motd;
        this.Player = Player;
        this.MineProtocol = MineProtocol;
        this.EaglerSkins = EaglerSkins;
        this.Util = Util;
    ...
    async _findPlugins(dir) {
        ...
        const lsRes = await Promise.all((await fs.readdir(dir))
            ...
            .map(async (res) => [pathUtil.join(dir, res), await fs.stat(pathUtil.join(dir, res))]));
        for (const [path, details] of lsRes) {
            if (details.isFile()) {
                ...
                try {
                    ...
                    ret.set(metadata.id, Object.assign({ path: pathUtil.resolve(path) }, metadata));
                }
                catch (err) {
                    this._logger.warn(`Failed to load plugin metadata file at ${metadataPath}: ${(_a = err.stack) !== null && _a !== void 0 ? _a : err}`);
    ...
    async _loadPlugins(plugins, order) {
        ...
        for (const id of order) {
            ...
            try {
                ...
                this._logger.warn(`Failed to load plugin entry point for plugin (${pluginMeta.name}) at ${pluginMeta.path}: ${(_a = err.stack) !== null && _a !== void 0 ? _a : err}`);
                this._logger.warn("This plugin will skip loading due to an error.");
            }
            return successCount;
        }
